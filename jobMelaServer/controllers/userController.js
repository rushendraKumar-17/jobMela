    ...
    if (email === "superadmin@gmail.com") {
      if (password === "1234") {
        const token = await jwt.sign(
          ...
          process.env.JWT_SECRET || "scaleorange123"
        );
        ...
        return res.statu(400).json({ message: "Invalid credentials" });
      }
    }
    const adminExists = await adminModel.findOne({ where: { email } });
    // console.log(adminExists);

    //check for admin if exists in adminModel
    if (adminExists) {
...
const getProfile = async (req, res) => {
  ...
  try {
    ...
    // decoded vuntundha super ADMIN ku        radhu token create chesa constant values tho   okk done done inka ee comments theeseyali anthe okkk

    if (decoded.company) {
      if (decoded.email === "superadmin@gmail.com") {
        return res.status(200).json({
          user: {
            company: "scaleorange",
            email: "superadmin@gmail.com",
